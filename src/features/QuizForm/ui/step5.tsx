...
export const Step5:React.FC<IStep1> = ({stepChange}) => {
    const {profileLink, setSProfileLink} = quizStore()
        ...
        <>
          {
            <div>
                ...
                <button className="w-full redGradient text-white rounded-[10px] text-sm flex items-center justify-center p-[8px] mt-3" onClick={() => validate(profileLink)}>Next</button>
            </div>
          }
      </>
  )
